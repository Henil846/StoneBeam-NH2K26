...
function generateRandomUpdate() {
    const types = ['quotation', 'order', 'notification', 'system'];
    const actions = ['new', 'updated', 'deleted'];
    ...
    const updateData = {
        ...
        type: type,
        action: action,
        data: generateUpdateData(type, action),
...
// Generate update data based on type and action
function generateUpdateData(type, action) {
    switch (type) {
        case 'quotation':
            return {
                contractorName: ['Elite Builders', 'Premium Construction', 'Quick Build'][Math.floor(Math.random() * 3)],
                projectTitle: ['Kitchen Renovation', 'Bathroom Remodel', 'Living Room Design'][Math.floor(Math.random() * 3)],
                amount: ['₹' + (Math.floor(Math.random() * 10 + 1) * '00,000')].join('')
            };
        case 'order':
            return {
        ...
        case 'notification':
            return {
                title: ['System Update', 'New Feature', 'Maintenance Notice'][Math.floor(Math.random() * 3)],
                message: ['New features available', 'Server maintenance tonight', 'Security update required'][Math.floor(Math.random() * 3)]
            };
        case 'system':
            return {
                message: ['Server performance optimized', 'Database updated', 'Security scan completed'][Math.floor(Math.random() * 3)]
            };
        default:
            return {
                message: 'Random system update'
            };
    }
}
...
server.on('upgrade', (request, socket, head) => {
    ...
    socket.writeHead(101, {
        'Upgrade': 'websocket',
        'Connection': 'Upgrade',
        'Sec-WebSocket-Accept': request.headers['sec-websocket-key']
    });

    socket.end('WebSocket upgrade not implemented in this simple version');
});
